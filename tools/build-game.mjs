...
if (!debug) {
  ...
  const packer = new Packer([{data:script,type:'js',action:'eval'}],{allowFreeVars:true,modelRecipBaseCount:11,modelMaxCount:4,numAbbreviations:28,sparseSelectors:[0,1,2,3,6,7,13,25,42,115,193,464],precision:14,recipLearningRate:930});
  const packed = packer.makeDecoder();
  script = assets.join('') + packed.firstLine + packed.secondLine;
  style = style.replace('*{box-sizing:border-box}', '').replace(/\s*([{}:;,])\s*/g, '$1').trim();
  html = html.replace(/>\s+</g, '><').replace(/<\/?(?:html|head|body)>/g,'').replace(/"(utf-8|icon|data:,|game)"/g,'$1').trim();
}
