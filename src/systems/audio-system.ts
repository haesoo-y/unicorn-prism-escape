...
export class AudioSystem {
  ...
  update(state: GameState): void {
    ...
    if (this.next < now + .08) {this.music(state.stage, this.step++, this.next); this.next += [60/132/2,60/144/2,60/156/2,60/168/2][state.stage<2?0:state.stage<5?1:state.stage<8?2:3]!}
    ...
    if (event & 4) {this.tone(150,.13,.4,'sawtooth',now,900); this.tone(160,.18,.35,'sine',now+.08,45)}
    if (event & 8) {this.tone(2400,.14,.45,'square',now,90);this.tone(90,.12,.25,'sine',now,40)}
    if (event & 16) for (let i=0;i<3;i++) this.tone([440,554,659][i]!,.18,.13,'sine',now+i*.06);
    if (event & state.abilities & 32) this.tone(660,.5,.3,'sine',now,55);
  ...
  private music(stage: number, step: number, at: number): void {
    ...
    if(!(beat&1)||phase>1)this.tone(root/2*(scale[(chord+(beat%4===2?4:0))%7]!),.16,.065,'triangle',at);
    if(!(beat&3))for(let i=0;i<3;i++)this.tone(root*(scale[(chord+i*2)%7]!),.4,.025,'triangle',at+i*.012);
    let note=melody[beat]!;if(alternate&&beat===10)note--;if(phase===3&&note<0)note=4;
    if(note>=0){const f=root*2*(scale[note]!);this.tone(f,beat%4===3?.1:.19,.052,'triangle',at);if(phase)this.tone(f/2,.15,.019,'sine',at+.025)}
    if(beat===7||beat===15)this.tone(root*(scale[phase+1]!)*2,.12,.028,'sine',at);
  ...
  private tone(frequency: number, duration: number, volume: number, type: OscillatorType, at: number, end = frequency): void {
    ...
    gain.gain.setValueAtTime(.0001, at); gain.gain.exponentialRampToValueAtTime(volume, at + .01); if(volume>=.3)gain.gain.setValueAtTime(volume,at+duration*.3); gain.gain.exponentialRampToValueAtTime(.0001, at + duration);
